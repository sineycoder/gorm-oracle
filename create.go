	...
	if stmt.SQL.Len() == 0 {
		...
		if hasConflict {
			...
			case reflect.Slice, reflect.Array:
				...
				hasBatch = true
				stmt.AddClauseIfNotExists(clause_.Insert{
					Insert: clause.Insert{Table: clause.Table{Name: stmt.Schema.Table}},
					Values: values,
					From:   clause.From{Tables: []clause.Table{{Name: DualTableName()}}},
				})
				stmt.Build("INSERT")
			default:
				// just 1 data
