...
func (m Migrator) CurrentDatabase() (name string) {
	_ = m.DB.Raw(
		fmt.Sprintf(`SELECT ORA_DATABASE_NAME as "Current Database" FROM %s`, DualTableName()),
	).Row().Scan(&name)
	return
}

func (m Migrator) CreateTable(dsts ...interface{}) error {
	for _, dst := range dsts {
		if err := m.TryQuoteReservedWords(dst); err != nil {
			return err
		}
		if err := m.TryRemoveOnUpdate(dst); err != nil {
			return err
		}
	}
	return m.Migrator.CreateTable(dsts...)
}

func (m Migrator) DropTable(dsts ...interface{}) error {
	dsts = m.ReorderModels(dsts, false)
...
func (m Migrator) DropColumn(dst interface{}, name string) error {
	if !m.HasColumn(dst, name) {
		return nil
	}
...
func (m Migrator) TryQuoteReservedWords(values ...interface{}) error {
